...
uint32_t Load4bCI (uint8_t *dst, uint8_t *src, int wid_64, int height, int line, int real_width, int tile)
{
  ...
  if (rdp.tlut_mode == 2)
  {
    load4bCI (src, dst, wid_64, height, line, ext, (uint16_t *)pal);
    
    return (1 << 16) | GR_TEXFMT_ARGB_1555;
  }

    load4bIAPal (src, dst, wid_64, height, line, ext, (uint16_t *)pal);
